'''
Dynamic representation learning by direct optimization
'''
...
from training import run_epoch_drl_dir, resume_checkpoint_dual, save_checkpoint_dual, adversarial
...
def main_worker(gpu, ckp_dir, cfg, sp_cfg, args):
    ...
    # train model
    epoch = start_epoch
    lambda_sp = sp_cfg['criterion']['lambda']
    scale = sp_cfg['criterion']['scale'] if 'scale' in sp_cfg['criterion'] else False
    shuffle = sp_cfg['shuffle'] if 'shuffle' in sp_cfg else False
    if not args.evaluate:
        for epoch in range(start_epoch, sp_cfg['optimizer']['epochs']):
            ...
            run_epoch_drl_dir(epoch, True, model, model_sp, dataloaders['train'], lambda_sp,
                              criterion, criterion_sp, optimizer, args, scale, shuffle=shuffle,
                              fprint=fprint, writer=writer, freeze_bn=freeze_bn)
            
            # save checkpoint
            if not args.ddp or gpu == 0:
                acc1 = run_epoch_drl_dir(epoch, False, model, model_sp, dataloaders['test'], lambda_sp,
                                         criterion, criterion_sp, None, args, scale, shuffle=shuffle,
                                         fprint=fprint, writer=writer)['Acc@1']
    ...
    # final test
    if not args.ddp or gpu == 0:
        met = run_epoch_drl_dir(epoch, False, model, model_sp, dataloaders['test_final'], lambda_sp,
                                criterion, criterion_sp, None, args, shuffle=shuffle, fprint=fprint, writer=writer)

...
class ResNet(nn.Module):
    ...
    def feat(self, x, ret_feat=[]):
        ...
        if ret_feat:
            feat = {'conv1': x1, 'conv2': x2, 'conv3': x3, 'conv4': x4, 'conv5': x5, 'pool': x}
            x = {k: feat[k] for k in ret_feat + ['pool']}
...
class ResNet2D(VideoNet):
    ...
    def feat(self, x, ret_feat=False):
        if len(x.shape) == 5:                           # bs * n_channels * n_frames * h * w
            x = x.transpose(1, 2)                       # bs * n_frames * n_channels * h * w
            N, D = x.shape[:2]
            y = self.net.feat(x.flatten(0, 1), ret_feat)     # (bs * n_frames) * feat_dim
            if ret_feat:
                y = {k: v.view(N, D, *v.shape[1:]).mean(1) for k, v in y.items()}
            else:
                y = y.view(N, D, *y.shape[1:]).mean(1)      # bs * feat_dim

        else:                                           # bs * n_channels * h * w
            y = self.net.feat(x, ret_feat)                   # bs * feat_dim
        
        return y


def resnet2d(n_class, model_depth, pretrained=False, progress=False, **kwargs):
    ...
    if model_depth == 18:
        ...
        raise Exception('model_depth={} unsupported'.format(model_depth))

    return ResNet2D(model, out_dim=n_class, **kwargs)
